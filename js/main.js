/* ============================================
   Main Site Logic
   (Updated: Search Sync + Maintenance Mode)
   ============================================ */
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ PC এবং Mobile সার্চ ইনপুট সিঙ্ক
  ...
  if (searchInputPC) {
    searchInputPC.addEventListener("input", debounce((e) => {
      syncSearchInputs('pc', e.target.value.trim());
    }, 450));
  }
  
  if (searchInputMobile) {
    searchInputMobile.addEventListener("input", debounce((e) => {
      syncSearchInputs('mobile', e.target.value.trim());
    }, 450));
  }
...
function productCardHTML(p) {
  ...
  const sourceBadge = `<span class="source-badge ${getSourceBadgeClass(source)}">${getSourceLabel(source)}</span>`;
  const discountBadge = discount > 0 ? `<span class="discount-badge">-${discount}%</span>` : "";

  let actionButtons = "";
  
  if (source === "daraz") {
    actionButtons = `<button class="buy-now-btn" onclick="requireAuthThenRedirect('${(p.affiliateLink || "").replace(/'/g, "\\'")}', '${p.id}', 'daraz')">এখনই কিনুন — Daraz 🛒</button>`;
  } else if (source === "external") {
    const socialLinks = p.socialLinks || {};
    actionButtons = `<div class="social-btns">`;
    if (socialLinks.facebook) actionButtons += `<button class="social-btn fb" onclick="trackSocialClick('${p.id}','facebook'); window.open('${socialLinks.facebook.replace(/'/g, "\\'")}','_blank')"><i class="fab fa-facebook"></i></button>`;
    if (socialLinks.messenger) actionButtons += `<button class="social-btn msg" onclick="trackSocialClick('${p.id}','messenger'); window.open('${socialLinks.messenger.replace(/'/g, "\\'")}','_blank')"><i class="fab fa-facebook-messenger"></i></button>`;
    if (socialLinks.whatsapp) actionButtons += `<button class="social-btn wa" onclick="trackSocialClick('${p.id}','whatsapp'); window.open('${socialLinks.whatsapp.replace(/'/g, "\\'")}','_blank')"><i class="fab fa-whatsapp"></i></button>`;
    if (socialLinks.website) actionButtons += `<button class="social-btn" onclick="trackSocialClick('${p.id}','website'); window.open('${socialLinks.website.replace(/'/g, "\\'")}','_blank')"><i class="fas fa-globe"></i></button>`;
    if (!socialLinks.facebook && !socialLinks.messenger && !socialLinks.whatsapp && !socialLinks.website) actionButtons += `<span style="font-size:0.78rem;color:var(--text-muted);">কোনো লিংক নেই</span>`;
    actionButtons += `</div>`;
  } else if (source === "own") {
    actionButtons = `<button class="cod-btn" onclick="requireAuthForCOD('${p.id}', '${escapeHTML(p.name).replace(/'/g, "\\'")}', ${p.currentPrice}, 'own')">📦 ক্যাশ অন ডেলিভারি</button>`;
  }

  return `
    <div class="product-card">
      <div class="product-img-wrap">
        ${sourceBadge} ${discountBadge}
      ...
      <div class="product-body">
        ...
        <div class="product-actions">${actionButtons}</div>
      </div>
    </div>`;
}
...
function renderCategories() {
  ...
  if (!wrap) return;
  const pills = [`<button class="category-pill ${activeCategory === "all" ? "active" : ""}" data-cat="all">✨ সব</button>`].concat(
    allCategories.map((c) => `<button class="category-pill ${activeCategory === c.id ? "active" : ""}" data-cat="${c.id}">${c.icon ? `<i class="${c.icon}"></i>` : ""} ${escapeHTML(c.name)}</button>`)
  );
...
function renderSourceTabs() {
  ...
  if (!wrap) return;
  let tabs = [`<button class="source-tab ${activeSource === "all" ? "active" : ""}" data-source="all">🌐 সব সোর্স</button>`];
  if (enabledSources.includes("daraz")) tabs.push(`<button class="source-tab ${activeSource === "daraz" ? "active" : ""}" data-source="daraz"><span class="source-dot" style="background:${getSourceColor('daraz')};"></span> Daraz</button>`);
  if (enabledSources.includes("external")) tabs.push(`<button class="source-tab ${activeSource === "external" ? "active" : ""}" data-source="external"><span class="source-dot" style="background:${getSourceColor('external')};"></span> অন্যান্য</button>`);
  if (enabledSources.includes("own")) tabs.push(`<button class="source-tab ${activeSource === "own" ? "active" : ""}" data-source="own"><span class="source-dot" style="background:${getSourceColor('own')};"></span> আমাদের</button>`);
...
function renderSourceFilter() {
  ...
  wrap.innerHTML = allSourcesConfig.map(s => `
    <label class="source-filter-item" onclick="event.stopPropagation();">
      <input type="checkbox" value="${s.id}" ${enabledSources.includes(s.id) ? "checked" : ""} onchange="toggleSourceFilter('${s.id}', this.checked)">
      <span class="source-dot" style="background:${s.color};"></span> ${s.label}
    </label>
  `).join("");
}

function toggleSourceFilter(sourceId, checked) {
  if (checked) { if (!enabledSources.includes(sourceId)) enabledSources.push(sourceId); }
  else { enabledSources = enabledSources.filter(s => s !== sourceId); }
  setActiveSources(enabledSources);
  renderSourceTabs();
  if (!enabledSources.includes(activeSource) && activeSource !== "all") activeSource = "all";
  renderProducts();
}
...
function setupTelegramPopup() {
  ...
  if (!closed) setTimeout(() => overlay.classList.add("active"), 1200);
